...
- PostgreSQL 16 via SQLAlchemy ORM (`database/models.py`)
...
- **Unset means `false`.** Every read defaults to off (`backend/middleware/auth.py`,
  `backend/main.py`, `backend/services/auth_service.py`), so anywhere without a
  `.env` — CI, containers, production — enforces auth. `env.example` ships
  `DEV_MODE=true`, and `setup_dev.sh` copies it, which is why a local checkout
  behaves as though it were the default. Tests that rely on a developer's `.env`
  (e.g. for `JWT_SECRET_KEY`, which is required once `DEV_MODE` is off) will fail
  in CI — set what you need explicitly
