...
The harness is `ai/core/`, per `docs/migration-plan.md` §4 WS-A. `core` rather
...
a seam a workflow can swap. With the loop in `ai/core/loop.ts`, `loop.ts` imports
...
`ai/tests/core/domain-free.test.ts` greps every file under `ai/core/` for domain
vocabulary and refuses any import from `ai/workflows/`. ADR-0002's requirement
...
The conformance workflow (`ai/workflows/tally/`) is deliberately useless and is
