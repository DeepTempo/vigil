...
Five contracts, in `services/agent/contracts/` and `infra/database/init/`.

### 1. The `Tool` port — `services/agent/contracts/tool.ts`
...
### 2. Event kinds and the `agent_events` discriminant — `services/agent/contracts/events.ts`
...
### 3. `Budget` and shared `Spend` — `services/agent/contracts/budget.ts`
...
### 4. The BullMQ job payload — `services/agent/contracts/job.ts`
...
### 5. The rate table — `infra/database/init/20_model_rates.sql`, `services/agent/contracts/rates.ts`
...
`services/agent/tests/contracts.test.ts`, so loosening either contract fails `typecheck`
...
agree. They are separately versioned lines — 2.15.x on PyPI, 5.x on npm — so
there is no matching version number to keep them in step, only a shared key
layout and a shared set of Lua scripts. The PyPI side is capped by `redis`
rather than chosen: bullmq 2.16 and later require `redis>=6`, `arq` requires
`redis<6`, and `arq` runs the existing llm-worker, so anything newer makes
`requirements.txt` unsolvable. Both are therefore exact pins, both are excluded
from automated dependency updates, and the walking skeleton's integration test
is the only thing that would catch a divergence, which is why it runs in CI
rather than only locally. The Python package is additionally classified alpha.
