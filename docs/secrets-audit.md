...
| Per-provider LLM keys (OpenAI, Ollama, custom Anthropic) | [services/llm_router.py:209](../services/llm_router.py), [services/worker/jobs.py](../services/worker/jobs.py) | GH #88 design: `LLMProviderConfig.api_key_ref` in DB points at a `secrets_manager` key (e.g. `llm_provider_anthropic-team_api_key`). **Keys themselves never land in the DB.** Settings UI → `backend/api/llm_providers.py` writes via `set_secret()`. |
