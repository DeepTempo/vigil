...
|---|---|---|
| `ANTHROPIC_API_KEY` / `CLAUDE_API_KEY` | [core/llm/harness/claude.py](../core/llm/harness/claude.py), [backend/services/ai_insights_service.py:29](../backend/services/ai_insights_service.py), [core/llm/router/router.py](../core/llm/router/router.py) | Default Anthropic provider. `get_secret()` layers env → dotenv → keyring. |
| Per-provider LLM keys (OpenAI, Ollama, custom Anthropic) | [core/llm/router/router.py](../core/llm/router/router.py), [services/worker/jobs.py](../services/worker/jobs.py) | GH #88 design: `LLMProviderConfig.api_key_ref` in DB points at a `secrets_manager` key (e.g. `llm_provider_anthropic-team_api_key`). **Keys themselves never land in the DB.** Settings UI → `backend/api/llm_providers.py` writes via `set_secret()`. |
